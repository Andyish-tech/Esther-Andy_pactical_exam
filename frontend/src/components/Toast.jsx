...
export default function Toast({ message, type = 'success', onClose }) {
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-4 rounded-xl border border-black shadow-lg animate-fade-in ${bgColors[type] || bgColors.success}`}>
      {icons[type]}
      <span className="text-sm font-semibold">{message}</span>
      <button onClick={onClose} className="ml-4 text-gray-400 hover:text-black transition-all shrink-0">
